...
export function sanitizeMessage(message: string, fallback: string) {
  ...
  if (m.includes('Insufficient capital in wallet')) {
    return 'Insufficient funds to activate this protocol. Fund the stake to proceed.'
  }
  ...
  if (match) return 'Insufficient funds to activate this protocol. Fund the stake to proceed.'
