                    ...
                    {[1, 2, 3, 4].map((_, i) => (
                        <div key={i} className="flex items-center gap-8 sm:gap-12 px-4 sm:px-6">
                            ...
                            <span className="flex items-center gap-2 text-[8px] sm:text-[9px] font-black uppercase italic text-red-500 tracking-widest text-left">
                                <span className="h-1.5 w-1.5 rounded-full bg-red-500" /> GOAL BREACH: ₦25K FORFEITED
                            </span>
                            <span className="flex items-center gap-2 text-[8px] sm:text-[9px] font-black uppercase italic text-white/40 tracking-widest text-left">
                                <span className="h-1.5 w-1.5 rounded-full bg-blue-500" /> NEW GOAL INITIALIZED: ₦50K
                            </span>
                            ...
                            <span className="flex items-center gap-2 text-[8px] sm:text-[9px] font-black uppercase italic text-white/40 tracking-widest text-left">
                                <span className="h-1.5 w-1.5 rounded-full bg-purple-500" /> CITIZEN SARAH COMPLETED WEEKLY GOAL
                            </span>
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-16 sm:mb-24">
                    ...
                    <p className="mt-6 sm:mt-8 text-white/40 text-[10px] sm:text-xs font-black italic uppercase tracking-tight max-w-2xl">
                        YOU DEFINE THE COST OF YOUR FAILURE. LOCKEDIN ALLOWS YOU TO CALIBRATE THE PAIN PROTOCOL BASED ON THE SEVERITY OF YOUR GOAL.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-10">
                    {[
                        { 
                            ...
                            desc: "DEFINE YOUR TARGET GOAL AND LOCK YOUR CAPITAL IN THE SECURE PROTOCOL VAULT. YOUR STAKE IS NOW BINDING.",
                            icon: <Shield className="text-blue-500" />
                        },
                        { 
                            step: "02", 
                            title: "Prove the Work", 
                            desc: "CONSISTENCY IS THE ONLY WAY. UPLOAD PHOTO PROOF OF YOUR PROGRESS EVERY 24 HOURS TO YOUR PRIVATE DASHBOARD.",
                            icon: <Camera className="text-[#ff7a00]" />
                        },
                        { 
                            step: "03", 
                            title: "Extract or Forfeit", 
                            desc: "COMMUNITY WITNESSES VERIFY YOUR PROGRESS. COMPLETED GOALS UNLOCK YOUR CAPITAL AND EARN YOU SYSTEM REWARDS.",
                        ...
                        <div className="relative p-8 sm:p-12 md:p-16 rounded-[2rem] sm:rounded-[4rem] bg-[#0a0f1a] border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden">
                            ...
                            <div className="space-y-8 sm:space-y-10">
                                <div className="p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] bg-white/[0.02] border border-white/5 shadow-inner group hover:bg-white/[0.04] transition-colors">
                                    ...
                                    <p className="text-[9px] sm:text-[10px] text-white/30 leading-relaxed font-black italic uppercase text-balance">FORFEITED PRINCIPAL FROM GOAL BREACHES IS POOLED AND SYNCHRONIZED. PERFECT ADHERENCE CITIZENS RECEIVE THESE REWARDS AS NON-MONETARY PROTOCOL CREDITS AND STATUS MULTIPLIERS.</p>
                                </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start gap-16 lg:gap-20 mb-16 sm:mb-24 border-b border-white/5 pb-12 sm:pb-16">
                    <div className="max-w-md w-full">
                        ...
                        <p className="text-xs sm:text-sm md:text-base text-white/30 font-black italic uppercase tracking-tight mb-8 sm:mb-12 leading-relaxed text-balance">
                            LOCKEDIN IS A BEHAVIORAL ENFORCEMENT GOAL FOR THOSE WHO REFUSE TO LIVE MEDIOCRE LIVES.
                        </p>
                        ...
                        <div className="space-y-6 sm:space-y-10">
                            ...
                            <ul className="space-y-4 sm:space-y-6 text-[10px] font-black uppercase tracking-widest text-white/60">
                                <li onClick={() => setShowModal({ 
                                    title: 'Architecture', 
                                    content: 'I. NON-CUSTODIAL MVP MODEL\nLockedin does not provide a stored-value wallet. Each goal is funded per vault using Paystack.\n\nII. PLATFORM LAYER\nLockedin is an instruction and accountability layer: goals, check-ins, witness approvals, penalty breakdowns, and reward credits.\n\nIII. BACKEND\nCore logic runs on Convex (schema, queries/mutations/actions, http endpoints, crons).\n\nIV. DATA MODEL\nVaults hold principal metadata, goals define requirements, goal logs capture evidence, and penalty events provide a full breakdown.\n\nV. WITNESS NETWORK\nAccountability partners approve/reject logs and gain access to full vault specifications.\n\nVI. ENVIRONMENT ALIGNMENT\nVercel + Convex + Paystack webhooks must point to the same deployment to avoid split-brain errors.' 
                                })} className="hover:text-blue-500 cursor-pointer transition-colors">Architecture</li>
                                
                                <li onClick={() => setShowModal({ 
                                    title: 'Security', 
                                    content: 'I. IDENTITY GATES\nEmail verification is required before accessing the user platform.\n\nII. PAYMENT SAFETY\nPaystack webhook signature verification is enforced and the server blocks test/live mode mismatches.\n\nIII. ACCESS CONTROL\nVault specifications are preview-only unless you are the owner or an active witness.\n\nIV. AUDITABILITY\nManual admin overrides require a reason and are recorded in an audit log.\n\nV. DATA PROTECTION\nEvidence images are stored securely and served via signed URLs.' 
                                })} className="hover:text-blue-500 cursor-pointer transition-colors">Security</li>
                                
                                <li onClick={() => setShowModal({ 
                                    title: 'API Docs', 
                                    content: 'I. CURRENT STATE\nThe MVP does not expose a public developer API.\n\nII. INTERNAL CONTRACTS\nThe frontend talks to Convex functions (queries/mutations/actions) and uses TanStack React Query.\n\nIII. WEBHOOKS\nPaystack webhook endpoint: /paystack-webhook (Convex HTTP endpoint).\n\nIV. ROADMAP\nA public API + documentation portal will be introduced after compliance review and stability hardening.' 
                        ...
                        <div className="space-y-6 sm:space-y-10">
                            ...
                            <ul className="space-y-4 sm:space-y-6 text-[10px] font-black uppercase tracking-widest text-white/60">
                                <li onClick={() => setShowModal({ 
                                    ...
                                    content: 'I. NOT FINANCIAL ADVICE\nLockedin is an accountability product. It is not an investment, savings, or deposit product and does not promise returns.\n\nII. STAKE-PER-VAULT FUNDING\nEach goal is funded per vault using Paystack. Lockedin does not provide a stored-value wallet in MVP.\n\nIII. CONSENT TO PENALTY RULES\nYou authorize the platform to apply penalties according to the rules you selected when creating the vault. Penalties accrue during the vault and settle at end-of-vault.\n\nIV. TRANSPARENCY\nA detailed penalty timeline is displayed inside each vault so you can see exactly what was missed and what penalty was applied.\n\nV. WITNESS REVIEW\nWitness decisions affect log outcomes. You are responsible for choosing witnesses you trust.\n\nVI. FEES\nPayment processing fees may apply and are charged by Paystack.\n\nVII. RISK\nUse amounts you can afford to commit. Digital services may experience downtime or delays.\n\nVIII. DISPUTES\nDisputes are handled under the laws of the Federal Republic of Nigeria.' 
                                })} className="hover:text-blue-500 cursor-pointer transition-colors">Terms</li>
                                
                                <li onClick={() => setShowModal({ 
                                    title: 'Privacy Protocol', 
                                    content: 'I. DATA MINIMIZATION\nWe collect only what is needed to operate the platform (account identity, goal configuration, and evidence logs).\n\nII. EVIDENCE VISIBILITY\nEvidence photos and notes are visible only to you and your active witnesses (and admins for support/operations). We do not use evidence for training AI models.\n\nIII. THIRD PARTIES\nWe use vetted third-party providers for payments and identity verification (e.g., Paystack, Mono). We do not sell your data.\n\nIV. COOKIES\nWe use functional session tokens to keep you authenticated.\n\nV. RETENTION\nOperational logs and audit records are retained for security and compliance purposes.\n\nVI. SECURITY INCIDENTS\nIf a security incident occurs, we will notify affected users through available channels.' 
                                })} className="hover:text-blue-500 cursor-pointer transition-colors">Privacy</li>
                                
                                <li onClick={() => setShowModal({ 
                                    title: 'BVN Usage Policy', 
                                    content: 'I. PURPOSE\nBVN verification is used for identity verification and fraud prevention.\n\nII. NO BANK BALANCE ACCESS\nBVN verification does not grant Lockedin access to your bank balance or transaction history.\n\nIII. PROVIDER\nVerification is performed via third-party identity providers (e.g., Mono) and is subject to their terms.\n\nIV. STORAGE\nWe do not store raw BVN in the application database.\n\nV. COMPLIANCE\nWe aim to comply with applicable Nigerian data protection regulations (NDPR/NDPA) and will update policies as requirements evolve.' 
